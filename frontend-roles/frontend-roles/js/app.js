...
const API_BASE = "https://n-capas.onrender.com";
...
function renderTabla(roles) {
  if (roles.length === 0) {
    tbodyRoles.innerHTML =
      '<tr><td colspan="3" class="empty">No hay roles registrados.</td></tr>';
    return;
  }
...
formCrear.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    setMsg(
      msgCrear,
      `Rol "${nuevo.nombre_rol}" creado con ID ${nuevo.id_rol}.`,
      "success",
    );
...
btnConfirmar.addEventListener("click", async () => {
  ...
  try {
    ...
    setMsg(
      msgLista,
      `Rol ID ${rolPendienteId} eliminado correctamente.`,
      "success",
    );
