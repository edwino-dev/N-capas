#los CRUD
#el rpeositorio es el encargado de hacer la conexion con la base de datos
...
class baseRepository:
    def __init__(self, model: Type, db:Session):
        #model es la clase del (clases del models)
        #db es la sesion de la base de datos
        self.model = model
        self.db = db
        pass
    
    def get_all(self, skip: int =0, limit: int =100) -> List:
        #offset salta los primeros N,limit limita el numero de resultados devueltos
        #skip es el numero de registros a saltar
        return self.db.query(self.model).offset(skip).limit(limit).all()
    
    def get_by_id(self, record_id: int) -> Optional:
